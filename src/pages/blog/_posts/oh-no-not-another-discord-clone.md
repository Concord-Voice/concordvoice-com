...
- Can you see the code? Open, source-available, or closed-source. Is the product independently runnable today, or a black box you rent?
...
- Nobody profits from your data, because nobody can see it. No ads. No client-side, behavioral, or identifying telemetry. Optional aggregate operational metrics support capacity and reliability; they do not include user identities, content, or behavior. We actually ripped Sentry back out (that's its own Dev Series story). No data business. Subscriptions are the model. E2EE-everywhere isn't just a privacy feature; it's a business-model commitment we can't quietly walk back later.
- Source-available today, and it outlives us. Concord is source-available under our [CVSL 1.0 license](/license), which converts to AGPL-3.0 on its Change Date. If the company gets hit by a bus, the project doesn't die with it.
- Self-hosting status: Concord Voice source is public; the supported self-hosted installer and deployment guide are not publicly released yet. Commercial self-hosting requires a [Concord Voice commercial license](/commercial-license). [Interested in a self-hosted early-access pilot? Contact us.](mailto:contact-us@concordvoice.com?subject=Self-hosted%20early-access%20pilot) It is an operator-assisted pilot, not a supported downloadable installer.
...
- Mobile and broader deployment work remain future roadmap items. We'll announce supported releases when they are ready.
